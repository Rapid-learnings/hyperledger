...
queryRawAndDryStock(){
    ...
    sudo docker exec -it cli-manufacturer-1 peer chaincode query --channelID mfd-whs-channel --name mwcc -c '{"Args":["availableDriedStock"]}'
}
...
queryRawDryAndRoastedStock(){
    ...
    sudo docker exec -it cli-manufacturer-1 peer chaincode query --channelID mfd-whs-channel --name mwcc -c '{"Args":["availableRoastedStock"]}'
}
...
queryRawDryRoastedAndFinishedStock(){
    ...
    sudo docker exec -it cli-manufacturer-1 peer chaincode query --channelID mfd-whs-channel --name mwcc -c '{"Args":["availableFinishedStock"]}'
}
package(){
    ...
    sudo docker exec -it cli-manufacturer-1 peer chaincode query --channelID mfd-whs-channel --name mwcc -c '{"Args":["getTotalPackages"]}'
...
dispatch(){
    ...
    sudo docker exec -it cli-manufacturer-1 peer chaincode query --channelID mfd-whs-channel --name mwcc -c '{"Args":["getWharehouseStock"]}'
}

placeOrderRetailer(){
    echo "############### CC call to query stock from mfd-whs-channel ###############"
    sudo docker exec -it cli-wharehouse-1 peer chaincode query --channelID whs-rtlr-channel --name wrcc -c '{"Args":["returnWarehouseSTockAccordingToPMCC"]}'

    sleep 6

    echo "############### updte wrcchouse stock ###############"
...
queryRetail(){
    ...
    sudo docker exec -it cli-retail-1 peer chaincode query --channelID whs-rtlr-channel --name wrcc -c '{"Args":["getRetailerStock"]}'
