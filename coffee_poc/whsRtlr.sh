...
queryWhsStock(){
    ...
    sudo docker exec -it cli-retail-1 peer chaincode query -C whs-rtlr-channel -n wr -c '{"Args":["returnWarehouseSTockAccordingToPMCC"]}'
}

queryRetStock(){
    echo "###################### querying retail stock ########################"
    sudo docker exec -it cli-retail-1 peer chaincode query -C whs-rtlr-channel -n wr -c '{"Args":["getRetailerStock"]}'
}
