...
export class wrcc extends Contract {
    ...
    async returnWarehouseSTockAccordingTomwCC(ctx: Context) {
    ...
    async updateWarehouseStock(ctx: Context, qty: string, flag: number) {
    ...
    async availableWarehouseStock(ctx: Context) {
    ...
    async getRetailerBalance(ctx: Context){
        let balBytes = await ctx.stub.getState(retailerBalance);
        let bal = parseInt(balBytes.toString());
        return bal;
    }

    async updateRetailerBalance(ctx: Context, amt: number, flag: number){
        let bal = 0;
    ...
    async getRetailerStock(ctx: Context){
        let stockBytes = await ctx.stub.getState(retailerStock);
        let stock = parseInt(stockBytes.toString());
        return stock;
    }

    async updateRetailerStock(ctx: Context, qty: number, flag: number){
        let stock = await this.getRetailerStock(ctx);
        if(flag == 0){
    ...
    async placeOrder(ctx: Context, qty: number, country: string, state: string) {
        // const clientMSPID = await ctx.clientIdentity.getMSPID();
        ...
        let txID = await ctx.stub.getTxID();
        if(rawStock < qty){
            throw new CustomError("Wharehouse Stock Is Less Than The Asked Qty,  current = ", rawStock);
        }
        
        let currentRetailerFunds = await this.getRetailerBalance(ctx);
        let amt = qty * pricePerPackage;
        if(amt > currentRetailerFunds){
            throw new CustomError("Retailer Funds Are Insufficient, current = ", currentRetailerFunds);
        }
        ...
        // Store order details
        await ctx.stub.putState(orderNo.toString(), Buffer.from(JSON.stringify(order).toString()));
        await ctx.stub.setEvent("place-order", Buffer.from(JSON.stringify(order).toString()));
        return txID;
    }
    ...
    async getWareHouseBalance(ctx: Context) {
        const clientMSP = await ctx.clientIdentity.getMSPID();
        if (clientMSP === 'tatastoreMSP') {
    ...
    async updateWarehouseBalance(ctx: Context, amt: number, flag: number){
    ...
    async getCurrentOrderNumber(ctx: Context) {
        const orderNoBytes = await ctx.stub.getState(orderNumber);
