...
mfdPrdCC(){
    ...
    sudo docker exec -it cli-manufacturer-1 peer lifecycle chaincode package ./chaincode/pmcc.tar.gz --path /opt/gopath/src/github.com/hyperledger/fabric/peer/chaincode/pmcc --lang node --label pmcc_1
...
mfdWhsCC(){
    ...
    # sudo docker exec -it cli-manufacturer-1 peer lifecycle chaincode package basic.tar.gz --path /opt/gopath/src/github.com/hyperledger/fabric/peer/chaincode/mfc-prdc --lang node --label basic_1.0
    sudo docker exec -it cli-manufacturer-1 peer lifecycle chaincode package ./chaincode/mwcc.tar.gz --path /opt/gopath/src/github.com/hyperledger/fabric/peer/chaincode/mwcc --lang node --label mwcc_1

    sleep 6

    # sudo docker exec -it cli-wharehouse-1 peer lifecycle chaincode package mwcc.tar.gz --path /opt/gopath/src/github.com/hyperledger/fabric/peer/chaincode/mwcccc --lang node --label mwcc_1
    ...
    sudo docker exec -it cli-wharehouse-1 peer lifecycle chaincode install /opt/gopath/src/github.com/hyperledger/fabric/peer/chaincode/mwcc.tar.gz
...
whsRtlrCC(){
    ...
    sudo docker exec -it cli-wharehouse-1 peer lifecycle chaincode package ./chaincode/wrcc.tar.gz --path /opt/gopath/src/github.com/hyperledger/fabric/peer/chaincode/wrcc --lang node --label wrcc_1

    sleep 6

    echo "********** ChainCode Packaged for Retailer-WhareHouse-Channel ********************"
    echo "********* Installing CC for Retailer-Wharehouse-Channel ************ "
    sudo docker exec -it cli-wharehouse-1 peer lifecycle chaincode install /opt/gopath/src/github.com/hyperledger/fabric/peer/chaincode/wrcc.tar.gz
...
#   export CC_PACKAGE_ID=$(sed -n "/${CC_NAME}_${CC_VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" mwcc.txt)
#   export CC_PACKAGE_ID
...
ApproveCCWhsRtlr(){
  sudo docker exec -it cli-wharehouse-1 peer lifecycle chaincode queryinstalled > wrcc.txt 2>&1 
  cat wrcc.txt
...
echo "Checking Commit Readiness for Channel whs-rtlr-channel"
    sudo docker exec -it cli-wharehouse-1 peer lifecycle chaincode checkcommitreadiness --channelID whs-rtlr-channel --name wrcc --version 1.0 --sequence 1 --init-required --output json
sleep 8
}
