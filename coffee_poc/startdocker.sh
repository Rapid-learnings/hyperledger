...
mfdWhsChannel(){
    ...
    echo "====== Updating Anchor peers by cli-wharehouse-1 mfd-whs-channel ======="
    sudo docker exec -it cli-wharehouse-1 peer channel update -o orderer1.gov.io:7050 --channelID mfd-whs-channel -f /opt/gopath/src/github.com/hyperledger/fabric/peer/channel-artifacts/ts-mfd-whs-anchor.tx --tls --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/gov.io/orderers/orderer1.gov.io/msp/tlscacerts/tlsca.gov.io-cert.pem
}

whsRtlrChannel(){
    # create whs-rtlr channel
    echo "create whs-rtlr channel"
    sudo docker exec -it cli-wharehouse-1 peer channel create -o orderer1.gov.io:7050 -c whs-rtlr-channel -f /opt/gopath/src/github.com/hyperledger/fabric/peer/channel-artifacts/whs-rtlr-channel.tx --outputBlock /opt/gopath/src/github.com/hyperledger/fabric/peer/channel-artifacts/whs-rtlr-genesis.block --tls --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/gov.io/orderers/orderer1.gov.io/msp/tlscacerts/tlsca.gov.io-cert.pem

    sleep 10

    # Joining channel whs-2 peer
    echo "Joining channel whs-2 peer"
    sudo docker exec -it cli-wharehouse-1 peer channel join -b /opt/gopath/src/github.com/hyperledger/fabric/peer/channel-artifacts/whs-rtlr-genesis.block

    sleep 10

    echo "====== Updating Anchor peers by cli-wharehouse-1 for whs-rtlr-channel ======="
